...
model.param.set('wlPML', 'wl/sqrt(abs(nClad^2-(nCore*R/(R+dr))^2))', 'Radial wavelength in PML');
...
model.sol('sol2').feature('e1').set('rtol', '1.0E-23');
...
model.result.export('data1').set('filename', ['E:\Users\kklong\Documents\' native2unicode(hex2dec({'76' '7e'}), 'unicode')  native2unicode(hex2dec({'5e' 'a6'}), 'unicode')  native2unicode(hex2dec({'4e' '91'}), 'unicode') '\Simulation Work\EC-FMF\cutoffMultibendsweep\plot.csv']);
...
outdata.neffBend = mphglobal(model,'real(ewfd2.neff)/r0');